...
public class SignActivity extends AppCompatActivity {
    ...
    private class signListener implements View.OnClickListener{
        ...
        public void signClick(View v) {
            ...
            if (isFormular(username, password)) {
                //signed = true;
            ...
            /*try {
                ...
                FileOutputStream f = new FileOutputStream(file, false); // 以覆盖方式输入

                f.write("SignActivity.signed:".getBytes());
                f.write((String.valueOf(signed)+"\n").getBytes());   // 将状态信息和“空格”写进文件中
                f.close();  // 关闭读写流
            }
            catch(IOException e)
            {
                Log.i("SignActivity1", "无法写入文件");
                e.printStackTrace();
            }*/
                ...
                Toast.makeText(SignActivity.this, "输入的用户名或密码不合法", Toast.LENGTH_SHORT).show();
                // 显示提示信息
                tvWarning.setText("给出提示信息");
            }
        }
        ...
        // 用户名和密码是否符合文法
        boolean isFormular(String username, String password) {
            return true;
        }
    }
